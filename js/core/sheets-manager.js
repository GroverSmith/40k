...
class SheetsManager {
	 ...
	 */
	// Updated clearAllCaches method in sheets-manager.js
	static clearAllCaches() {
		// Use CacheManager for unified cache clearing
		const clearedCount = CacheManager.clearAll();
		
		// Also clear any legacy embeds caches
		Object.values(this.embeds).forEach(embed => embed.clearCache());
		
		console.log(`Cleared ${clearedCount} cache entries via CacheManager`);
	}
