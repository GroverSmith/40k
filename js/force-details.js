...
class ForceDetailsApp {
    ...
    displayBattleHistory(battles) {
        ...
        html += '<th>Date</th><th>Opponent</th><th>Battle Size</th><th>Score</th><th>Result</th>';
        html += '</tr></thead><tbody>';
        
        battles.forEach(battle => {
            const date = battle['Date Played'] ? new Date(battle['Date Played']).toLocaleDateString() : 'Unknown';
            const resultClass = battle.result === 'Victory' ? 'victory' : 
                              battle.result === 'Defeat' ? 'defeat' : 'draw';
            
            html += `
                <tr>
                    <td>${date}</td>
                    <td>${battle.opponent} (${battle.opponentPlayer})</td>
                    <td>${battle['Battle Size'] || '-'} pts</td>
                    <td>${battle.forceScore || 0} - ${battle.opponentScore || 0}</td>
