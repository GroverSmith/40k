// filename: story-armies-gas-script.js
// Google Apps Script for Story_Armies Junction Table
// Deploy this as a web app

const SPREADSHEET_ID = '1YbPSfXMJro_x9d1W18RyZ4MQyfhoOwjgtDR0RhSju1E';
const SHEET_NAME = 'xref_story_armies';
...
function cascadeDeleteByParent(parentTable, parentKey) {
  ...
  if (!sheet) {
    console.log('Story armies sheet not found');
...
function doPost(e) {
  try {
    ...
    // Default operation is create story-army relationships
    const { storyKey, armyKeys } = data;
    
    if (!storyKey || !armyKeys || !Array.isArray(armyKeys)) {
      throw new Error('storyKey and armyKeys array are required');
    }
    ...
    if (!sheet) {
      console.log('Creating story armies sheet');
      sheet = spreadsheet.insertSheet(SHEET_NAME);
      
      const headers = ['story_key', 'army_key', 'timestamp', 'deleted_timestamp'];
    ...
    // Create rows for each army key
    if (armyKeys.length > 0) {
      const rows = armyKeys.map(armyKey => [
        storyKey,
        armyKey.trim(),
        timestamp,
        '' // Deleted Timestamp
    ...
    return ContentService
      .createTextOutput(JSON.stringify({
        success: true,
        message: 'Story-Army relationships created',
        count: armyKeys.length
      }))
      .setMimeType(ContentService.MimeType.JSON);

  } catch (error) {
    console.error('Error in story-armies doPost:', error);
    return ContentService
      .createTextOutput(JSON.stringify({
        success: false,
        error: error.message
...
function doGet(e) {
  try {
    ...
    switch(action) {
      case 'list':
        return getStoryArmiesList();
      default:
        return getStoryArmiesList();
    }
      
  } catch (error) {
    console.error('Error in story-armies doGet:', error);
    return ContentService
      .createTextOutput(JSON.stringify({
        success: false,
        error: error.message
...
function getStoryArmiesList() {
  try {
    ...
    console.log('getStoryArmiesList - Total active rows found:', rows.length);
    console.log('getStoryArmiesList - Headers:', headers);
    
    // Convert to objects with consistent field names
    const storyArmies = rows.map((row) => {
      const obj = {};
    ...
    console.log('getStoryArmiesList - Returning story-armies:', storyArmies.map(storyArmy => ({ 
      storyKey: storyArmy['story_key'], 
      armyKey: storyArmy['army_key']
    })));
    
    return ContentService
      .createTextOutput(JSON.stringify({
        success: true,
        count: storyArmies.length,
        totalCount: storyArmies.length,
        data: storyArmies,
        hasMore: false
      }))
      .setMimeType(ContentService.MimeType.JSON);
    ...
    console.error('Error getting story-armies list:', error);
