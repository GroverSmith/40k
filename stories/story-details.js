...
/**
 * Load and display a single story by key
 */
async function loadStoryDetails() {
    ...
    if (!storyKey) {
        container.innerHTML = '<div class="error-message">No story specified</div>';
        return;
    }
    ...
    try {
        ...
        if (cachedAll && cachedAll.valid && cachedAll.data) {
            story = findStoryInCache(cachedAll.data, storyKey);
            if (story) {
        ...
        if (!story && cachedRecent && cachedRecent.valid && cachedRecent.data) {
            story = findStoryInCache(cachedRecent.data, storyKey);
            if (story) {
        ...
        container.innerHTML = '<div class="error-message">Error loading story</div>';
    }
}
 ...
 */
function findStoryInCache(data, storyKey) {
    if (!data) return null;
        ...
        <article class="story-article">
            <header class="story-header">
                ...
                <div class="story-meta">
                    ...
                    <span class="story-date">Posted: ${formatDate(story['Timestamp'])}</span>
                </div>
            </header>
